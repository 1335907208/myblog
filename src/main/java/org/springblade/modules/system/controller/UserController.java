...
public class UserController {
	...
	public R submit(@Valid @RequestBody User user) {
		if (Func.isEmpty(user.getTenantId()) || "".equals(user.getTenantId())) {
	 ...
	 */
	@PostMapping("/confirm")
	public R confirm(@Valid @RequestBody UserAgent userAgent){
		UserAgent agent = userAgentService.getById(userAgent.getId());
		if(Func.isNotEmpty(agent)){
			BladeUser user = SecureUtil.getUser();
			agent.setAuthResult(Func.toStr(user.getUserId()));
			agent.setAuthTime(LocalDateTime.now());
			agent.setStatus(1);
			return R.status(userAgentService.updateById(agent));
		}
